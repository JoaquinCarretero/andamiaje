...
export function MonthlyReportForm() {
  ...
  const { saveStatus, lastSaved, triggerSave } = useAutoSave<typeof formData>({
    onSave: async (data) => {
      // Simulación de guardado en backend
      await new Promise(resolve => setTimeout(resolve, 500))
      localStorage.setItem('monthlyReport_draft', JSON.stringify({
        ...data,
  ...
  useEffect(() => {
    const savedDraft = localStorage.getItem('monthlyReport_draft')
    if (savedDraft) {
      try {
  ...
  useEffect(() => {
    const hasSignificantData = formData.studentName.trim() || 
                               formData.month.trim() ||
                               formData.activitiesPerformed.trim()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmed = await confirm({
      ...
      description: "Una vez enviado, el reporte será revisado por el coordinador. El borrador se eliminará.",
      type: "info",
    ...
    if (confirmed) {
      ...
      localStorage.removeItem('monthlyReport_draft')
    }
  }
