...
import { Card, CardContent, CardHeader, CardTitle, Button, Input, Textarea, Label, SaveIndicator, useAutoSave, useConfirmation } from "@/ui"
...
export function WorkPlanForm() {
  ...
  // ✅ Hook de confirmación para envío del formulario
  const { confirm, ConfirmDialog } = useConfirmation()

  // ✅ Hook de auto-save con debounce de 2 segundos
  const { saveStatus, lastSaved, triggerSave } = useAutoSave({
    onSave: async (data: typeof formData) => {
      ...
      localStorage.setItem('workPlan_draft', JSON.stringify({
        ...data,
  ...
  useEffect(() => {
    const savedDraft = localStorage.getItem('workPlan_draft')
    if (savedDraft) {
      try {
        const draft = JSON.parse(savedDraft)
        // No sobrescribir la fecha de guardado
  ...
  // ✅ Auto-save cuando cambian los datos (solo si hay datos significativos)
  // Usar ref para evitar loop infinito
  const lastSavedDataRef = useRef<string>("");

  useEffect(() => {
    const hasSignificantData = formData.patientName.trim() || 
                               formData.professionalName.trim() ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmed = await confirm({
      title: "¿Enviar Plan de Trabajo?",
      description: "Una vez enviado, el plan será revisado por el coordinador. Puedes continuar editando el borrador.",
      type: "info",
      confirmText: "Enviar para Revisión",
      cancelText: "Cancelar",
      showIcon: true,
    })
    ...
    if (confirmed) {
      ...
      localStorage.removeItem('workPlan_draft')
    }
  }
  ...
  return (
    <>
      ...
      {/* ✅ Dialog de confirmación para envío */}
      <ConfirmDialog />
    </>
  )
}